...
## Part 5: Работа с useRef

1. Компонент ClickTimer

   * Время первого клика фиксируется в Ref
   * Счетчик кликов хранится в ref, не вызывая ререндер
   * В консоли отображается разница во времени и общее количество

2. Компонент PreviousInput

   * Текущее значение отслеживается через onChange
   * Предыдущее значение сохраняется через ref
   * UI корректно отображает предыдущее значение

3. Компонент FocusTracker

   * Установка фокуса работает через ref.current.focus()
   * Переход фокуса отслеживается через onFocus
   * Количество фокус-переходов корректно отображается в консоли

4. Дополнительные задачи - не реализовывал
